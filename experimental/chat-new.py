...
class Client:
    ...
    def receive():
        ...
        global GameStateGameOver2, GameStateInGame, LetterBox, num, num2, num3, num4
        coords1 = 0.48
        coords2 = 0.75
        coords3 = 0.48
        coords4 = 0.005
        ...
        while True:
            try:
                receive_data = clientSocket.recv(20)
            except:
            ...
            if not receive_data:
                ...
                if '(.)=(.)quit' in receive_data.decode():
                    ...
                    coords1 = receive_data.decode().strip('&^&*^')
                    coords1 = float(coords1)
                    num = coords1
                    MySpacer.place(relx=coords1, rely=coords2)

                elif '&^&*!' in receive_data.decode():
                    coords2 = receive_data.decode().strip('&^&*!')
                    coords2 = float(coords2)
                    num2 = coords2
                    MySpacer.place(relx=coords1, rely=coords2)

                elif '&-_*^' in receive_data.decode():
                    coords3 = receive_data.decode().strip('&-_*^')
                    coords3 = float(coords3)
                    num3 = coords3
                    EnemySpacer.place(relx=coords3, rely=coords4)

                elif '&-_*!' in receive_data.decode():
                    coords4 = receive_data.decode().strip('&-_*!')
                    coords4 = float(coords4)
                    num4 = coords4
                    EnemySpacer.place(relx=coords3, rely=coords4)
                    ...
                    if state2 == 'join':
                        ...
                        boxX = receive_data.decode().strip('!@:/|!@:/|!@:/-!')
                        boxX = float(boxX)

                elif '!@:/|!@:/|!@:/|!' in receive_data.decode():
                    if state2 == 'join':
                        print(receive_data.decode())
                        boxY = receive_data.decode().strip('!@:/|!@:/|!@:/|!')
                        boxY = float(boxY)

                elif '!@:/|!@:/|!@:/*!---' in receive_data.decode():
                    if state2 == 'join':
                        print(receive_data.decode())
                        fdelay = receive_data.decode().strip('!@:/|!@:/|!@:/*!---')
                        fdelay = int(fdelay)

                elif '!@:/|!@:/|!@:/&!---' in receive_data.decode():
                    if state2 == 'join':
                        print(receive_data.decode())
                        fdelay2 = receive_data.decode().strip('!@:/|!@:/|!@:/&!---')
                        fdelay2 = int(fdelay)
                        _thread.start_new_thread(Window.handleRandomBoxes, (boxX, boxY, fdelay, fdelay2))
                    ...
                    try:
                        ...
                        if state == 'host':
                            print('YE2')
                            coderec = receive_data.decode().strip('{}%^%$')
                            if coderec == str(HOST_CODE):
                                Client.send('GS*()STARTSTARTSTAR', True)
                                Window.teamSelection()
                    except:
                        pass

                elif 'GS*()STARTSTARTSTAR' in receive_data.decode():
                    if state == 'join':
                        Window.teamSelection()

                elif '{{!*£READYREADYREAD' in receive_data.decode():
                    if state == 'join':
                        if ready == True:
                            Window.startGame()

                elif '{{!*£READYREADYREA2' in receive_data.decode():
                    if state == 'host':
                        if ready == True:
                            Window.startGame()
...
class Window:
    ...
    def startGame():
        global state
        try:
            WaitingLabel.place_forget()
        except:
            pass
        if state == 'join':
            Client.send('{{!*£READYREADYREA2', True)
        state = 'game'
        Window.drawModels()

    @staticmethod
    def handleRandomBoxes(extNum2, extNum3, extDel, extDel2):
        global SpeedGood, SpeedBad, numer2, numer3, Speed2
        print('HELLO', extNum2, extNum3, extDel, extDel2)
        SpeedGood = Label(Selector, text='MOVEMENT SPEED INCREASED', fg='#2ecc71', font=('Segoe UI', 11, 'bold'), bg='#141414')
        SpeedBad = Label(Selector, text='MOVEMENT SPEED DECREASED', fg='#e74c3c', font=('Segoe UI', 11, 'bold'), bg='#141414')
        Speed2 = Button(Selector, text='✦', font='Arial 15 bold', bg=windowBackground, fg='#7f8c8d', bd=0, width=1, height=1)
        ...
        BNUM3 = extNum3
        numer4 = extDel2
        Speed2.place(relx=BNUM2, rely=BNUM3)
        time.sleep(numer4)
        Speed2.place_forget()
        resetNum()
    ...
    @staticmethod
    def makeRandomBoxes():
        global numer3, numer2
        global SpeedGood, SpeedBad, Speed
        SpeedGood = Label(Selector, text='MOVEMENT SPEED INCREASED', fg='#2ecc71', font=('Segoe UI', 11, 'bold'), bg='#141414')
        SpeedBad = Label(Selector, text='MOVEMENT SPEED DECREASED', fg='#e74c3c', font=('Segoe UI', 11, 'bold'), bg='#141414')
        Speed = Button(Selector, text='✦', font='Arial 15 bold', bg=windowBackground, fg='#7f8c8d', bd=0, width=1, height=1)

        def resetNum():
            global numer2, numer3
            numer2 = .999
            numer3 = .999

        while True:
            numer1 = random.randint(6, 9)
            numer2 = random.uniform(0.05, 0.85)
            numer3 = random.uniform(0.10, 0.85)
            numer4 = random.randint(4, 9)
            time.sleep(numer1)
            Client.send('!@:/|!@:/|!@:/-!{:0.2f}'.format(numer2), True)
            Client.send('!@:/|!@:/|!@:/|!{:0.2f}'.format(numer3), True)
            Client.send('!@:/|!@:/|!@:/*!---{0}'.format(numer1), True)
            Client.send('!@:/|!@:/|!@:/&!---{0}'.format(numer4), True)
            Speed.place(relx=numer2, rely=numer3)
            time.sleep(numer4)
            Speed.place_forget()
            resetNum()
    ...
    def drawModels():
        ...
        EnemySpacerP.place_forget()
        MySpacerP.place_forget()

        EnemySpacer = Label(Selector, text='■', font='Arial 25 bold', bg=windowBackground, fg='white', bd=0)
        EnemySpacer.place(relx=.48, rely=.005)

        MySpacer = Label(Selector, text='■', font='Arial 25 bold', bg=windowBackground, fg=ColoursHex[Cindex], bd=0, width=1, height=1)
        MySpacer.place(relx=.48, rely=.75)

        if state2 == 'host':
            _thread.start_new_thread(Window.makeRandomBoxes, ())

    @staticmethod
    def drawModelsPre():
        global EnemySpacerP, MySpacerP

        EnemySpacerP = Button(Selector, text='■', font='Arial 35 bold', bg=windowBackground, fg='white', bd=0)
        EnemySpacerP.place(relx=.53, rely=.38)

        MySpacerP = Button(Selector, text='■', font='Arial 35 bold', bg=windowBackground, fg=ColoursHex[Cindex], bd=0)
        MySpacerP.place(relx=.38, rely=.38)

    @staticmethod
    def selectTeam(colour):
        global Cindex
        Cindex = ColoursHex.index(colour)
        Selector.after(1, lambda: Colours[0].place_forget())
        Selector.after(200, lambda: Colours[1].place_forget())
        Selector.after(400, lambda: Colours[2].place_forget())
        Selector.after(600, lambda: Colours[3].place_forget())
        Selector.after(800, lambda: Colours[4].place_forget())
        Selector.after(1000, lambda: Colours[Cindex].place(relx=.45, rely=.3))
        Selector.after(1300, lambda: Colours[Cindex].place_forget())
        Selector.after(1600, lambda: Colours[Cindex].place(relx=.45, rely=.3))
        Selector.after(1900, lambda: Colours[Cindex].place_forget())
        Selector.after(2200, lambda: Colours[Cindex].place(relx=.45, rely=.3))
        Selector.after(2500, lambda: Colours[Cindex].place_forget())
        Selector.after(2500, lambda: Window.drawModelsPre())

        Window.readyPlayer()

    @staticmethod
    def checkReady():
        global ready, WaitingLabel
        WaitingLabel = Label(Selector, text='WAITING FOR OTHER PLAY TO READY', font=('Segoe UI', 14, 'bold'), bg=windowBackground, fg='white')
        WaitingLabel.place(relx=.05, rely=.125)
        Client.send('{{!*£READYREADYREAD', True)
        ready = True
        ReadyLabel.place_forget()
        ReadyButton.place_forget()

    @staticmethod
    def teamSelection():
        global Colours, ColoursHex
        global SelectLabel
        try:
            HostCode.place_forget()
            WaitingLabel.place_forget()
        except:
            pass
        try:
            ...
            sendArrow.place_forget()
        except:
            pass

        SelectLabel = Label(Selector, text='SELECT A TEAM COLOUR', font=('Segoe UI', 14, 'bold'), bg=windowBackground, fg='white')
        SelectLabel.place(relx=.05, rely=.125)

        Colour1 = Button(Selector, text='■', font='Arial 35 bold', bg=windowBackground, fg='#e74c3c', bd=0, command=lambda: Window.selectTeam('#e74c3c'))
        Colour1.place(relx=.15, rely=.3)

        Colour2 = Button(Selector, text='■', font='Arial 35 bold', bg=windowBackground, fg='#2ecc71', bd=0, command=lambda: Window.selectTeam('#2ecc71'))
        Colour2.place(relx=.30, rely=.3)

        Colour3 = Button(Selector, text='■', font='Arial 35 bold', bg=windowBackground, fg='#3498db', bd=0, command=lambda: Window.selectTeam('#3498db'))
        Colour3.place(relx=.45, rely=.3)

        Colour4 = Button(Selector, text='■', font='Arial 35 bold', bg=windowBackground, fg='#9b59b6', bd=0, command=lambda: Window.selectTeam('#9b59b6'))
        Colour4.place(relx=.6, rely=.3)

        Colour5 = Button(Selector, text='■', font='Arial 35 bold', bg=windowBackground, fg='#f39c12', bd=0, command=lambda: Window.selectTeam('#f39c12'))
        Colour5.place(relx=.75, rely=.3)

        Colours = [Colour1, Colour2, Colour3, Colour4, Colour5]
        ColoursHex = ['#e74c3c', '#2ecc71', '#3498db', '#9b59b6', '#f39c12']

    @staticmethod
    def readyPlayer():
        global ReadyButton, ReadyLabel
        SelectLabel.place_forget()

        ReadyLabel = Label(Selector, text='CLICK THE READY BUTTON', font=('Segoe UI', 14, 'bold'), bg=windowBackground, fg='white')
        Selector.after(2500, lambda: ReadyLabel.place(relx=.05, rely=.125))

        ReadyButton = Button(Selector, text='READY', font=('Segoe UI', 12, 'bold'), bg=windowBackground, fg=ColoursHex[Cindex], bd=0, command= lambda: (ReadyLabel.place_forget(), ReadyButton.place_forget(), Window.checkReady()))
        Selector.after(2500, lambda: ReadyButton.place(relx=.85, rely=.85))

    @staticmethod
    def gameselect():
        global EnemySpacer, MySpacer, selected, num, num2, numer2, numer3, incrementer, state, HOST_CODE, state2, Selector, ready, num3, num4
            ...
            if state == 'game':
                ...
                if state2 == 'host':
                    ...
                    if num3 > 0.05:
                        num3 -= incrementer
                if state2 == 'host':
                    clientSocket.send(str.encode('&^&*^{:0.13f}'.format(num)))
                else:
                    clientSocket.send(str.encode('&-_*^{:0.13f}'.format(num3)))
        ...
        def downKey(event):
            if state == 'game':
                ...
                if state2 == 'host':
                    if num2 < 0.9:
                        num2 += incrementer + 0.005
                else:
                    if num4 < 0.9:
                        num4 += incrementer + 0.005
                if state2 == 'host':
                    clientSocket.send(str.encode('&^&*!{:0.13f}'.format(num2)))
                else:
                    clientSocket.send(str.encode('&-_*!{:0.13f}'.format(num4)))
        ...
        def enterKey(event):
            if state == 'game':
                global numer2, numer3, incrementer
                chances = [SpeedGood, SpeedBad]
                powerup = random.choice(chances)
                pIndex = chances.index(powerup)
                if state2 == 'host':
                    diffx = numer2 - num
                    diffy = numer3 - num2
                    ...
                    if diffx < 0.06:
                        if diffy < 0.06:
                            Speed.place_forget()
                            Selector.after(1, lambda: powerup.place(relx=.05, rely=.125))
                            Selector.after(2000, lambda: powerup.place_forget())
                            if pIndex == 0:
                                incrementer = 0.02
                            else:
                                incrementer = 0.005
        ...
        Selector.title('Game BETA')

        Selector.bind('<Left>', leftKey)
        Selector.bind('<Right>', rightKey)
        Selector.bind('<Up>', upKey)
        Selector.bind('<Down>', downKey)
        Selector.bind('<Return>', enterKey)

        def makeRandomBoxes():
            global numer3, numer2, Speed
            global SpeedGood, SpeedBad
            SpeedGood = Label(Selector, text='MOVEMENT SPEED INCREASED', fg='#2ecc71', font=('Segoe UI', 11, 'bold'), bg='#141414')
            SpeedBad = Label(Selector, text='MOVEMENT SPEED DECREASED', fg='#e74c3c', font=('Segoe UI', 11, 'bold'), bg='#141414')
            Speed = Button(Selector, text='✦', font='Arial 15 bold', bg=windowBackground, fg='#7f8c8d', bd=0, width=1, height=1)
            ...
            while True:
                ...
                numer2 = random.uniform(0.05, 0.85)
                numer3 = random.uniform(0.1, 0.85)
                numer4 = random.randint(4, 9)
                Speed.place(relx=numer2, rely=numer3)
                time.sleep(numer4)
                Speed.place_forget()
        ...
        def joinScreen():
            global state, state2, JoinButton, enterCode, sendArrow
            state = 'join'
            state2 = 'join'

            HostButton.place_forget()
            JoinButton.place_forget()

            JoinButton.place(relx=.27, rely=.45)

            enterCode = Entry(Selector, width=10, highlightthickness=2, bg='#141414', fg='white', font=('Segoe UI', 11, 'bold'))
            enterCode.config(highlightbackground='#FFFFFF')
            enterCode.place(relx=.45, rely=.451)

            sendArrow = Button(Selector, text='→', bg='#141414', fg='white', font=('Segoe UI', 18, 'bold'), bd=0, command=lambda:Client.send('{}%^%$' + enterCode.get(), True))
            sendArrow.place(relx=.62, rely=.4)

        def hostScreen():
            global state, HOST_CODE, state2, HostCode, WaitingLabel
            state = 'host'
            state2 = 'host'
            HOST_CODE = random.randint(100000, 999999)
            HostButton.place_forget()
            JoinButton.place_forget()
            WaitingLabel = Label(Selector, text='WAITING FOR PLAYER', font=('Segoe UI', 14, 'bold'), bg=windowBackground, fg='white')
            WaitingLabel.place(relx=.05, rely=.125)

            HostCode = Label(Selector, text=HOST_CODE, font=('Segoe UI', 45, 'bold'), fg='#9b59b6', bg='#141414')
            HostCode.place(relx=.325, rely=.38)

        def hostJoin():
            global HostButton, JoinButton
            HostButton = Button(Selector, text='HOST MATCH', font=('Segoe UI', 11, 'bold'), fg='#9b59b6', bg='#141414', bd=0, command=lambda:hostScreen())
            HostButton.place(relx=.27, rely=.45)
            JoinButton = Button(Selector, text='JOIN MATCH', font=('Segoe UI', 11, 'bold'), fg='#2ecc71', bg='#141414', bd=0, command=lambda:joinScreen())
            JoinButton.place(relx=.57, rely=.45)
